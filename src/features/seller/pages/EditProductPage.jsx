...
const EditProductPage = () => {
  ...
  const categories = [
    ...
    'Книги', 'Дом и сад', 'Спорт', 'Игрушки', 
    'Красота и здоровье', 'Зоотовары'
  ]
    ...
    const loadProduct = async () => {
      try {
        ...
        if (product.sellerId !== user.id) {
          ...
          addToast('У вас нет прав для редактирования этого товара', 'error')
          return
        }
        setFormData({
          name: product.name || '',
  ...
  const validateForm = () => {
    ...
    if (!formData.price || Number(formData.price) <= 0) return 'Цена должна быть больше 0'
    if (!formData.stock || Number(formData.stock) < 0) return 'Остаток не может быть отрицательным'
    if (!formData.category) return 'Выберите категорию'
    if (formData.images.length === 0) return 'Добавьте хотя бы одно фото'
    if (formData.discount < 0 || formData.discount > 99) return 'Скидка должна быть от 0 до 99%'
    return null
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast('Товар успешно обновлён', 'success')
  ...
  const handleDelete = async () => {
    if (window.confirm('Удалить товар? Это действие нельзя отменить.')) {
      try {
        ...
        addToast('Товар успешно удалён', 'success')
        navigate('/seller/products')
      } catch (err) {
  ...
  if (initialLoading) return <div className={styles.loading}>Загрузка товара...</div>
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.section}>
            <h2>Фотографии</h2>
            <p className={styles.hint}>Добавьте до 5 фотографий. Поддерживаются JPG, PNG (до 5MB каждая)</p>
          ...
          {formData.category === 'Одежда' && (
            <div className={styles.section}>
              ...
              <button type="button" onClick={handleAddSize} className={styles.addBtn}>
              ...
              <button type="button" onClick={handleAddColor} className={styles.addBtn}>
          ...
          {formData.category === 'Электроника' && (
            <div className={styles.section}>
              ...
              <button type="button" onClick={handleAddSpecification} className={styles.addBtn}>
          ...
          <div className={styles.actions}>
            <button type="button" onClick={handleDelete} className={styles.deleteBtn}>
              <FiTrash2 /> Удалить товар
            </button>
            <div>
              <button type="button" onClick={() => navigate('/seller/products')} className={styles.cancelBtn}>
                Отмена
              </button>
